...
class Database(object):
    ...
    def fetchDocument(self, _id):
        """Fetch a document by its _id value."""
        sid = _id.split("/")
    ...
    def createGraph(self, name, createCollections=True):
        ...
        'name' must be the name of a class inheriting from Graph.
        You can decide weither or not you want non existing collections to be created by setting the value of 'createCollections'.
        If the value if 'false' checks will be performed to make sure that every collection mentioned in the edges definition exists.
    ...
    def hasCollection(self, name):
        """Return true if the databse has a collection by the name of 'name'."""
        return name in self.collections

    def hasGraph(self, name):
        """Return true if the databse has a graph by the name of 'name'."""
    ...
    def AQLQuery(self, query, batchSize=100, rawResults=False, bindVars={}, options={}, count=False, fullCount=False, json_encoder=None, **moreArgs):
        ...
        """
        return AQLQuery(self, query, rawResults=rawResults, batchSize=batchSize, bindVars=bindVars, options=options,
            count=count, fullCount=fullCount, json_encoder=json_encoder, **moreArgs)

    def explainAQLQuery(self, query, allPlans=False):
        """Returns an explanation of the query.

        Setting allPlans to True will result in ArangoDB returning all possible plans.
        False returns only the optimal plan.
        """
    ...
    def validateAQLQuery(self, query, bindVars={}, options={}):
        """Return the server answer is the query is valid, else raise an AQLQueryError."""
...
class DBHandle(Database):
    """As the loading of a Database also triggers the loading of collections and graphs within.

    Only handles are loaded first. The full database are loaded on demand in a fully transparent manner.
    """

    def __init__(self, connection, name):
        """Init DBHandle with connection and name."""
        self.connection = connection
        self.name = name
