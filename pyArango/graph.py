...
class Graph_metaclass(type):
    ...
    def isGraph(cls, name):
        """Return true/false depending if there is a graph called name."""
...
def getGraphClasses():
    """Return a dictionary of all defined graph classes."""
...
class EdgeDefinition(object):
    ...
    def toJson(self):
        """Return Json."""
        # Probably isn't really JSON here but a Python dict.
...
class Graph(with_metaclass(Graph_metaclass, object)):
    ...
    def createVertex(self, collectionName, docAttributes, waitForSync=False):
        """Add a vertex to the graph and returns it."""
    ...
    def deleteVertex(self, document, waitForSync=False):
        """Delete a vertex from the graph as well as al linked edges."""
    ...
    def unlink(self, definition, doc1, doc2):
        """Delete all links between doc1 and doc2."""
    ...
    def traverse(self, startVertex, **kwargs):
        ...
        see: https://docs.arangodb.com/HttpTraversal/README.html for a full list of the possible kwargs.
        The function must have as argument either:
            direction = "outbout"/"any"/"inbound"
        or
            expander = "custom JS (see arangodb's doc)".
