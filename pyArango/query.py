...
class RawCursor(object):
    """Raw interface to cursors that returns json."""
...
class Query(object):
    ...
    All query classes derive from it
    """
    ...
    def __next__(self):
        """Return next element of query result.

        Automatomatically calls for new batches if needed.
        """
        try:
            v = self[self.currI]
        except IndexError:
    ...
    def __getitem__(self, i):
        """Returns ith result of the query."""
        if not self.rawResults and (self.result[i].__class__ is not Edge and self.result[i].__class__ is not Document):
            self._developDoc(i)
        return self.result[i]

    def __len__(self):
        """Return number of elements in query results."""
        return len(self.result)

    def __getattr__(self, k):
        """Get attribute named k, and enable subscript getters like dict."""
        try:
...
class AQLQuery(Query):
    ...
    def __init__(self, database, query, batchSize, bindVars, options, count, fullCount, rawResults=True, json_encoder=None, **moreArgs):
        """Init AQLQuery."""
...
class SimpleQuery(Query):
    ...
    def __init__(self, collection, queryType, rawResults, json_encoder=None, **queryArgs):
        """Init SimpleQuery."""
